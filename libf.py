...
def poly(x, y, sig, deg):  # fiting polynomial model: x,y dataset; sig is uncertainity; deg is degree
    n = len(x)
    ...
    plt.plot(u,ff(u,deg,params),label="Fitted data")
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.legend()
    plt.show()
    return params, A_inv, chi/(n-deg)    # returning paramter array, covariance
                                        # and reduced chi-square
